...
require([
    ...
    ready(function () {//65.3447265625,4.21875, 135.7470703125, 53.61328125
        ...
        //---------Ext JS 事件----------
        //初始化选择图层
        comboBoxLayerSelect = Ext.getCmp('survey_TypicalEnterprise_LayerCombo');
        //图层选择变化时
        if (comboBoxLayerSelect) {
            comboBoxLayerSelect.on('change', onComboBoxChange);
        }
        ...
        function onMapLayerLoad(map) {
            if (!layer) {
                //comboBox = Ext.getCmp('survey_Landuse_LayerCombo');
                var store = Ext.data.StoreManager.lookup('systemMapQYYDLayerStore');
                if (store.getCount() > 0) {
                    var model = store.getAt(0);
                    comboBoxLayerSelect.select(model);
                }
            }
        }
        ...
        grid = Ext.getCmp('survey_TypicalEnterpriseGrid');
        if (grid) {
        ...
        //地图panel大小变化时
        mapPanel = Ext.getCmp('survey_TypicalEnterprise_MapPanel');
        if (mapPanel) {
